...
The app's Home screen sends you to the setup screen until an API URL and secret are stored, shows
"API is not responding" when it cannot reach the API, and "Change API URL or secret" when the API rejects the token. Point
...
4. Re-enter the secret on the app's setup screen. Until you do, every request answers `401` and the
   Home screen shows "Change API URL or secret".
