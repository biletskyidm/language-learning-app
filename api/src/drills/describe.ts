...
export class DescribeDrill implements DrillStrategy {
  ...
  async generate({ index, targets }: DrillContext): Promise<DrillRound> {
    ...
    return {
      ...
      targets: [target],
      material: {
        targetExpressionId: target.expressionId,
        expression: target.expression,
        candidates: targets.map(({ expression }) => expression),
      },
    }
  }

  async judge(round: DrillRound, body: unknown): Promise<DrillJudgement> {
    const { expression, candidates } = describeMaterialSchema.parse(round.material)
    ...
    const { guess, note } = await withRetry(() => this.llm.judgeDescription({ description, candidates }), {
      attempts: JUDGE_ATTEMPTS,
    })

    return { ok: true, answer: parsed.data, verdict: { guess, correct: norm(guess) === norm(expression), note } }
  }

  scoresFor(round: DrillRound): Map<string, number> {
    const { correct } = describeVerdictSchema.parse(round.verdict)
    const { targetExpressionId } = describeMaterialSchema.parse(round.material)

    return new Map([[targetExpressionId, correct ? DRILL_CORRECT_SCORE : DRILL_WRONG_SCORE]])
  }
  ...
  aggregate(rounds: DrillRound[]): DrillAggregates {
    ...
    return {
      ...
      correct: judged.filter(({ correct }) => correct).length,
      wrong: judged.filter(({ correct }) => !correct).length,
    }
  }
}
